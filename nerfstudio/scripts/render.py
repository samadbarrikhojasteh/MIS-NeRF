...
class BaseRender:
    ...
    colormap_options: colormaps.ColormapOptions = colormaps.ColormapOptions()
    # colormap_options2: colormaps.ColormapOptions = colormaps.ColormapOptions2()
    """Colormap options."""
...
class DatasetRender(BaseRender):
    ...
    def main(self):
        ...
        data_manager_config = config.pipeline.datamanager
        # assert isinstance(data_manager_config, VanillaDataManagerConfig)
        errors = {"depth_error_wo_bg": [], "depth_error_wo_bg_spe": [], "alpha_wo_bg": [], "alpha_wo_bg_spe": []}
                ...
                for camera_idx, (camera, batch) in enumerate(progress.track(dataloader, total=len(dataset))):
                    ...
                    for rendered_output_name in rendered_output_names:
                        ...
                        if is_raw:
                            ...
                            # depth_norm = self.normalize(output_image)
                            # output_image = torch.clip(output_image, 0, 1)
                            # output_image = output_image * outputs["accumulation"] + (1 - outputs["accumulation"])
                            ...
                            mask_dir = config.data / "mask_val_ERODE"
                            ...
                            output_disparity_wo_bg = self.output_path / split / "disparity_wo_bg"
                            output_disparity_wo_bg_spe = self.output_path / split / "disparity_wo_bg_spe"
                            output_depth_gt_masked = self.output_path / split / "gt_depth_masked"
                            output_disparity_wo_bg.mkdir(exist_ok=True, parents=True)
                            output_disparity_wo_bg_spe.mkdir(exist_ok=True, parents=True)
                            output_depth_gt_masked.mkdir(exist_ok=True, parents=True)
                            if "mask" in batch:  # This mask comes from .json file
                                mask = torch.cat((batch["mask"], batch["mask"], batch["mask"]), dim=2)
                                depth_mask = output_image * mask[:, :, 0:1]
                                output_image = torch.Tensor.cpu(depth_mask).numpy()
                                H, W, _ = mask.shape
                                # Get depth exr file and change their names and dimensions (according to the mask_train_ERODE)
                                if os.path.exists(config.data.parent/"depth"):
                                    if os.path.exists(str(config.data / "modified_depth")) is not True:
                                        self.resize(config.data, W, H)
                                    #select depth map from the new resized depth folder
                                    depth_path_new = (config.data/"modified_depth"/image_name).with_suffix(".exr")
                                    depth_GT_new = self.get_exr_rgb(str(depth_path_new))
                                    # depth_GT_resize_new = cv2.resize(depth_GT_new, [W, H]) # resize GT depth
                                    depth_GT = torch.tensor(depth_GT_new).cuda(0)[:, :, 0:1]
                                    # get depths and GT_depth without bg indices
                                    depth_GT_mask = depth_GT * mask[:, :, 0:1]
                                    # get depths and GT-depth without highlight indices
                                    depth_GT_mask_wo_spe = depth_GT.cuda(0) * specular_read[:, :, 0:1].cuda(0)
                                    depth_wo_spe = depth_mask * specular_read[:, :, 0:1].cuda(0)

                                    # sam = torch.Tensor.cpu(disparity_wo_bg_spe).numpy()

                                    # Reshape and compute alpha_bg between GT depth_bg and rendered depth_bg
                                    depth_resh = depth_mask[mask[:, :, 0:1]]
                                    depth_GT_resh = depth_GT_mask[mask[:, :, 0:1]].to(torch.float32)
                                    alpha_wo_bg = torch.tensordot(torch.transpose(depth_resh, -1, 0), depth_GT_resh, dims=1) / torch.tensordot(
                                        torch.transpose(depth_resh, -1, 0), depth_resh, dims=1)

                                    # Reshape and compute alpha_bg_spe and between GT depth_bg_spe and rendered depth_bg_spe
                                    depth_resh_bg_spe = depth_wo_spe[specular_read[:, :, 0:1]].to(torch.float32)  # only for computation RMSE and alpha
                                    depth_GT_resh_bg_spe = depth_GT_mask_wo_spe[specular_read[:, :, 0:1]].to(torch.float32) # only for computation RMSE and alpha
                                    alpha_wo_spe = torch.tensordot(torch.transpose(depth_resh_bg_spe, -1, 0),depth_GT_resh_bg_spe, dims=1) / torch.tensordot(
                                        torch.transpose(depth_resh_bg_spe, -1, 0), depth_resh_bg_spe, dims=1)

                                    # We apply disparity maps only to the depth NOT prop_depth_0 and prp_depth_1
                                    if rendered_output_name == "depth":
                                        # Disparity computation
                                        disparity_wo_bg = torch.mean(torch.abs(depth_GT_mask - depth_mask * alpha_wo_bg), dim=2)
                                        disparity_wo_bg_spe = torch.mean(torch.abs(depth_GT_mask_wo_spe - depth_wo_spe * alpha_wo_spe), dim=2)
                                        # Compute depth rmse
                                        rmse_depth_wo_bg = self.rmse(depth_GT_resh, depth_resh * alpha_wo_bg) * 1000 # Error for wo bg in mm
                                        rmse_depth_wo_bg_spe = self.rmse(depth_GT_resh_bg_spe, depth_resh_bg_spe * alpha_wo_spe) * 1000  # Error for wo bg & spe in mm
                                        errors["depth_error_wo_bg"].extend([np.round(torch.Tensor.cpu(rmse_depth_wo_bg).numpy(), 2)])
                                        errors["depth_error_wo_bg_spe"].extend([np.round(torch.Tensor.cpu(rmse_depth_wo_bg_spe).numpy(), 2)])
                                        errors["alpha_wo_bg"].extend([np.round(torch.Tensor.cpu(alpha_wo_bg).numpy(), 2)])
                                        errors["alpha_wo_bg_spe"].extend([np.round(torch.Tensor.cpu(alpha_wo_spe).numpy(), 2)])
                                        # Save disparity map
                                        self.plot_disparity({"name": "Depth error w/o bg", "rmse": rmse_depth_wo_bg,
                                                             "disparity": disparity_wo_bg, "adress": output_disparity_wo_bg / image_name.with_suffix(".png")})
                                        self.plot_disparity(
                                            {"name": "Depth error w/o bg_spe", "rmse": rmse_depth_wo_bg_spe,
                                             "disparity": disparity_wo_bg_spe, "adress": output_disparity_wo_bg_spe / image_name.with_suffix(".png")})
                                        # save GT_depth masked
                                        depth_GT_mask_np = torch.Tensor.cpu(depth_GT_mask).numpy()
                                        depth_GT_triple = np.concatenate([depth_GT_mask_np, depth_GT_mask_np, depth_GT_mask_np], axis=2)
                                        media.write_image(output_depth_gt_masked / image_name.with_suffix(".png"), depth_GT_triple, fmt="png")
                                    output_image = (output_image * alpha_wo_bg.cpu().numpy())
                                ...
                                output_image = torch.Tensor.cpu(output_image).numpy()
                                output_image = np.concatenate([output_image, output_image, output_image], axis=2)##
                        else:
                            output_image = (
                                colormaps.apply_colormap(
                            ...
                            if "mask" in batch:  # This mask comes from .json file
                                mask = torch.cat((batch["mask"], batch["mask"], batch["mask"]), dim=2)
                                output_image = torch.where(mask == 0.0, 1.0, output_image)
                                output_image = torch.Tensor.cpu(output_image).numpy()
        ...
        # # save the errors
        import pandas as pd
        error = pd.DataFrame.from_dict(errors, orient="columns")
        error.to_csv(self.output_path / split / "depth_errors.ods")
        table = Table(
            title=None,
            show_header=False,
            box=box.MINIMAL,
        ...
        if indx > 1:
            ...
            img_org = config.data / "images"
        # img_gt = config.data / "renders" / "test" / "gt-rgb"
        img_gt = self.output_path / split / "gt-rgb"
        # img_rendered = config.data / "renders" / "test" / "rgb"
        img_rendered = self.output_path / split / "rgb"
        img_names = sorted(os.listdir(img_gt))
        mask_bg = []
        mask_spe = []
        pred_img = []
        gt_img = []
        for img in img_names:
            img_gt_res = cv2.imread(str(img_gt / img))
            img_rend = cv2.imread(str(img_rendered / img))
            ...
            gt_img.append(img_gt_res)
        mask_spe_stack = np.stack(mask_spe, 0)
        mask_org_stack = np.stack(mask_bg, 0)
        img_gt_stack = np.stack(gt_img, 0)
        img_rendered_stack = np.stack(pred_img, 0)
        mask_spe_torch = torch.from_numpy(mask_spe_stack)
        mask_bg_torch = torch.from_numpy(mask_org_stack)
        img_pred_torch = torch.from_numpy(img_rendered_stack)
        img_gt_torch = torch.from_numpy(img_gt_stack)
        m = erode_mask(mask_spe_torch.float(), pixel_radius=3)  # eroded mask spe
        mask = torch.squeeze(mask_bg_torch * 1)
        masked = (m == 1) & (mask == 1)  # intergate bg mask and spe mask together
        mask_bg_spe = masked.to(torch.float32)
        ...
        lpips_bg = []
        lpips_spe = []
        for frame in range(0, len(mask_bg_spe)):
            # Compute psnr, ssim and lpips for images and points wo_bg
            mask_triple = torch.stack((mask[frame], mask[frame], mask[frame]), 2)
            predicted_rgb_wo_bg_psnr = img_pred_torch[frame][mask_triple.bool()]
            gt_rgb_wo_bg_psnr = img_gt_torch[frame][mask_triple.bool()]
            predicted_rgb_wo_bg = (img_pred_torch[frame] * mask_triple) / 255
            predicted_rgb_wo_bg = torch.moveaxis(predicted_rgb_wo_bg, -1, 0)[None, ...]
            gt_rgb_wo_bg = (img_gt_torch[frame] * mask_triple) / 255
            gt_rgb_wo_bg = torch.moveaxis(gt_rgb_wo_bg, -1, 0)[None, ...]
            # Compute psnr for wo_bg(pixels) and ssim , lpips for wo_bg(image)
            psnr_bg.append(np.array(psnr(predicted_rgb_wo_bg_psnr, gt_rgb_wo_bg_psnr)))
            ssim_bg.append(np.array(ssim(predicted_rgb_wo_bg.to(torch.float32), gt_rgb_wo_bg.to(torch.float32))))
            lpips_bg_res = lpips(predicted_rgb_wo_bg.to(torch.float32), gt_rgb_wo_bg.to(torch.float32))
            lpips_bg.append(torch.Tensor.detach(lpips_bg_res).numpy())
            # psnr_wo_bg = psnr(predicted_rgb_wo_bg_psnr, gt_rgb_wo_bg_psnr)
            # ssim_wo_bg = ssim(predicted_rgb_wo_bg.to(torch.float32), gt_rgb_wo_bg.to(torch.float32))
            # lpips_wo_bg = lpips(predicted_rgb_wo_bg.to(torch.float32), gt_rgb_wo_bg.to(torch.float32))

            # Compute psnr, ssim and lpips for images nad points wo_bg_spe
            mask_spe_triple = torch.stack((mask_bg_spe[frame], mask_bg_spe[frame], mask_bg_spe[frame]), 2)
            predicted_rgb_wo_specular_psnr = img_pred_torch[frame][mask_spe_triple.bool()]
            gt_rgb_wo_specular_psnr = img_gt_torch[frame][mask_spe_triple.bool()]
            predicted_rgb_wo_spe = (img_pred_torch[frame] * mask_spe_triple) / 255
            predicted_rgb_wo_spe = torch.moveaxis(predicted_rgb_wo_spe, -1, 0)[None, ...]
            gt_rgb_wo_spe = (img_gt_torch[frame] * mask_spe_triple) / 255
            gt_rgb_wo_spe = torch.moveaxis(gt_rgb_wo_spe, -1, 0)[None, ...]
            # Compute psnr for wo_bg_sp(pixels) and ssim , lpips for wo_bg(image)
            psnr_spe.append(np.array(psnr(predicted_rgb_wo_specular_psnr, gt_rgb_wo_specular_psnr)))
            ssim_spe.append(np.array(ssim(predicted_rgb_wo_spe.to(torch.float32), gt_rgb_wo_spe.to(torch.float32))))

            lpips_spe_res = lpips(predicted_rgb_wo_spe.to(torch.float32), gt_rgb_wo_spe.to(torch.float32))
            lpips_spe.append(torch.Tensor.detach(lpips_spe_res).numpy())
            # psnr_wo_spe = psnr(predicted_rgb_wo_specular_psnr, gt_rgb_wo_specular_psnr)
            # ssim_wo_spe = ssim(predicted_rgb_wo_spe.to(torch.float32), gt_rgb_wo_spe.to(torch.float32))
            # lpips_wo_spe = lpips(predicted_rgb_wo_spe.to(torch.float32), gt_rgb_wo_spe.to(torch.float32))
        result = {"psnr_bg": psnr_bg, "psnr_spe": psnr_spe, "ssim_bg": ssim_bg, "ssim_spe": ssim_spe,
                  "lpips_bg": lpips_bg,
                  "lpips_spe": lpips_spe}
        pd_version = pd.DataFrame(result)
        pd_version.to_csv(self.output_path / split / "metrics.ods")
        # sam = torch.Tensor.cpu(mask_spe_triple).numpy()

    def normalize(self, x):
        return (x - torch.min(x)) / (torch.max(x) - torch.min(x))

    def rmse(self, actual, pred):
        return torch.sqrt(torch.square(torch.subtract(actual, pred)).mean())

    def plot_disparity(self, dict_all):
        import matplotlib.pyplot as plt
        from mpl_toolkits.axes_grid1 import make_axes_locatable
        rmse = float(dict_all["rmse"])
        frame_error = dict_all["name"] +"="+ str(round(rmse, ndigits= 4))+"(Mm)"
        disparity = self.normalize(dict_all["disparity"]) # Normalization
        # disparity = dict_all["disparity"]
        ...
        im = ax.imshow(cv0)#,cmap='hsv')  # Here make an AxesImage rather than contour
        fig.colorbar(im, cax=cax)
        ax.set_title(frame_error)
        plt.savefig(str(dict_all["adress"]))
        return
    ...
    def get_exr_rgb(self, path):
        ...
        data = [np.fromstring(c, np.float32).reshape(size) for c in
                I.channels("RGB", Imath.PixelType(Imath.PixelType.FLOAT))]
        img = np.dstack(data)
        return img
    def resize(self, dir, W, H):
        ...
        import imageio
        i = 1
        if os.path.exists(str(dir / "modified_depth")) is not True: os.mkdir(
            str(dir / "modified_depth"))
        for t in sorted(os.listdir(dir.parent / "depth")):
            depth_path = (dir.parent / "depth" / t).with_suffix(".exr")
            depth_GT = self.get_exr_rgb(str(depth_path))
            depth_GT_resize = cv2.resize(depth_GT, [W, H])  # resize GT depth
            path = dir / "modified_depth" / f"frame_{i:05}.exr"
            imageio.imwrite(path, depth_GT_resize)
            i += 1
        return
