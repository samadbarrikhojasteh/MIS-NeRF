...
class PixelSampler:
    ...
    def sample_method(
        ...
        if isinstance(mask, torch.Tensor):
            ...
            #Add for pair pixels
            chosen_indices_pair = random.sample(range(len(nonzero_indices)), k=rays_to_sample)
            indices = nonzero_indices[chosen_indices_pair]
        else:
            indices = (
                torch.rand((batch_size, 3), device=device)
                * torch.tensor([num_images, image_height, image_width], device=device)
            ).long()
        #Add from the last Class(last function)
        # pair_indices_right = torch.hstack(
        #     (
        #         torch.zeros(rays_to_sample, 1, device=device, dtype=torch.long),
        #         torch.ones(rays_to_sample, 1, device=device, dtype=torch.long),
        #         torch.zeros(rays_to_sample, 1, device=device, dtype=torch.long),
        #     )
        # )
        #
        # pair_indices_left = torch.hstack(
        #     (
        #         torch.zeros(rays_to_sample, 1, device=device, dtype=torch.long),
        #         -torch.ones(rays_to_sample, 1, device=device, dtype=torch.long),
        #         torch.zeros(rays_to_sample, 1, device=device, dtype=torch.long),
        #     )
        # )
        #select right, left, up, down randomly
        right= torch.tensor((0, 1, 0))
        left = torch.tensor((0, -1, 0))
        up = torch.tensor((0, 0, 1))
        down = torch.tensor((0, 0, -1))
        chosen_direction = random.sample([right, left, up, down], k=1)[0]
        second_indices = indices+chosen_direction
        # pair_indices_right += indices
        # pair_indices_left += indices
        indices = torch.hstack((second_indices, indices)).view(rays_to_sample * 2, 3)
        return indices
    ...
    def collate_image_dataset_batch(self, batch: Dict, nonzero_indices, num_rays_per_batch: int, keep_full_image: bool = False):
        """
        ...
        if "mask" in batch:
            if self.config.is_equirectangular:
                ...
                indices = self.sample_method(
                    nonzero_indices, num_rays_per_batch, num_images, image_height, image_width, mask=batch["mask"], device=device
                )
        else:
            if self.config.is_equirectangular:
                indices = self.sample_method_equirectangular(
    ...
    def sample(self, image_batch: Dict, nonzero_indices):
        ...
        if isinstance(image_batch["image"], list):
            ...
            pixel_batch = self.collate_image_dataset_batch(
                image_batch, nonzero_indices, self.num_rays_per_batch, keep_full_image=self.config.keep_full_image
            )
        else:
