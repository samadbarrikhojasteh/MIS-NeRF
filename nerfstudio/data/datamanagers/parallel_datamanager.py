...
class DataProcessor(mp.Process):  # type: ignore
    ...
    def run(self):
        ...
        while True:
            batch = self.pixel_sampler.sample(self.img_data, self.nonzero_indices_pair)
    ...
    def one_time_nonzero(self):
        if os.path.exists(os.path.join(self.config.data, "mask_train_ERODE")):
            print("MASK PATH EXISTING JUST UPDATED THE MASKS")
        else:
            os.mkdir(os.path.join(self.config.data, "mask_train_ERODE"))

        maskit = []
        for i in range(len(self.img_data["image"])):
            mask_mean = cv2.cvtColor(self.img_data["image"][i].numpy(), cv2.COLOR_RGB2GRAY)
            mask_raw = np.where(mask_mean > 0.9, 0.0, 1.0) * 255.0
            maskit.append(mask_raw/255)
        mask_mean = np.stack(maskit, 0)
        mask_torch = torch.from_numpy(mask_mean)
        m = erode_mask(mask_torch.float(), pixel_radius=3)
        # Add the colmap mask to the highlight mask
        if "mask" in self.img_data:
          mask= torch.squeeze(self.img_data["mask"]*1)
          masked = (m == 1) & (mask == 1)
          masked = masked.to(torch.float32)
          self.img_data["mask"] = masked
          m= masked
        else:
            self.img_data["mask"] = m
        #writting the masks together
        mm= m[:, :, :, None]
        mmm= torch.permute(mm,(3, 0, 1, 2))
        for i in range(len(self.img_data["image"])):
            torchvision.utils.save_image(mmm[:,i,:,:], os.path.join(self.config.data,"mask_train_ERODE", os.path.split(self.dataparser_outputs.image_filenames[self.img_data["image_idx"][i]])[-1]))
        nonzero_indices_pair = torch.nonzero(m, as_tuple=False)
        self.nonzero_indices_pair = nonzero_indices_pair
...
class ParallelDataManager(DataManager, Generic[TDataset]):
    ...
    def praper_val_image(self):
        self.setup_eval()
        self.val_batch = next(self.iter_eval_image_dataloader)
    ...
    def next_eval(self, step: int) -> Tuple[RayBundle, Dict]:
        ...
        batch = self.eval_pixel_sampler.sample(image_batch, self.nonzero_indices_val_pair)
        ray_indices = batch["indices"]
    ...
    def one_time_nonzero(self):
        if os.path.exists(os.path.join(self.config.data, "mask_val_ERODE")):
            print("MASK_VAL PATH EXISTED JUST UPDATED MASK VAL IMAGES!")
        else:
            os.mkdir(os.path.join(self.config.data, "mask_val_ERODE"))

        maskit=[]
        for i in range(len(self.val_batch["image"])):
            mask_mean= cv2.cvtColor(self.val_batch["image"][i].numpy(), cv2.COLOR_RGB2GRAY)
            mask_raw = np.where(mask_mean > 0.9, 0.0, 1.0) * 255.0
            maskit.append(mask_raw/255)
        mask_mean= np.stack(maskit,0)
        mask_torch= torch.from_numpy(mask_mean)
        m = erode_mask(mask_torch.float(), pixel_radius= 3)
        # Add the colmap mask to the specular mask points
        if "mask" in self.val_batch:
            mask = torch.squeeze(self.val_batch["mask"] * 1)
            masked = (m == 1) & (mask == 1)
            masked = masked.to(torch.float32)
            self.val_batch["mask"] = masked
            m = masked
        else:
            self.val_batch["mask"] = m

        #writing the masks together
        mm = m[:, :, :, None]
        mmm = torch.permute(mm, (3, 0, 1, 2))
        for i in range(len(self.val_batch["image"])):
            torchvision.utils.save_image(mmm[:, i, :, :], os.path.join(self.config.data, "mask_val_ERODE",
                                                                       os.path.split(self.eval_dataparser_outputs.image_filenames[self.val_batch["image_idx"][i]])[-1]))
        nonzero_indices_val = torch.nonzero(m, as_tuple=False)
        self.nonzero_indices_val_pair = nonzero_indices_val
