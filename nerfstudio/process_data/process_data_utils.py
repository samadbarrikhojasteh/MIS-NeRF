...
def copy_before_mask(image_paths, image_dir):
    image_prefix = "frame_"
    image_paths = list_images(image_paths)
...
def copy_images_list(
    ...
    Args:
        ...
        mask_path: path of the mask that will be used in Colmap(optional)
    ...
    # Remove original directory and downscaled versions (for mask folder only)
    if mask_path is not None:
        mask_dir = image_dir.parent / "masks"
        for i in range(num_downscales + 1):
            dir_to_remove = mask_dir if i == 0 else f"{mask_dir}_{2 ** i}"
            shutil.rmtree(dir_to_remove, ignore_errors=True)
        mask_dir.mkdir(exist_ok=True, parents=True)

    image_dir.mkdir(exist_ok=True, parents=True)
    #Create images_colmap folder(just for using colmap not nerfstudio)
    #Colmap does not work well with masked images + mask
    #It should be original images+ mask
    images_colmap = image_dir.parent / "images_colmap"
    shutil.rmtree(images_colmap, ignore_errors=True)
    images_colmap.mkdir(exist_ok=True, parents=True)
    copied_image_paths = []
    ...
    for idx, image_path in enumerate(image_paths):
        ...
        copied_image_path = image_dir / f"{image_prefix}{idx + 1:05d}{image_path.suffix}"
        copied_image_colmap_path = images_colmap / f"{image_prefix}{idx + 1:05d}{image_path.suffix}"
        try:
            ...
            if image_path.suffix.lower() in ALLOWED_RAW_EXTS:
                ...
                if percent_radius != 1.0:
                    mask_path_rad = image_dir.parent / "masks"
                    mask_img = cv2.imread(str(mask_path_rad / "mask.png"))
                    img = cv2.imread(str(image_path))
                    img2 = np.where(mask_img == 255, img, 0)
                    cv2.imwrite(str(copied_image_path), img2)
                    shutil.copy(image_path, copied_image_colmap_path)
                else:
    ...
    #Copy and rescale the masks
    if mask_path is not None:
        copied_masks_paths = []
        for idx, m_path in enumerate(mask_path):
            if verbose:
                CONSOLE.log(f"Copying masks {idx + 1} of {len(mask_path)}...")
            copied_mask_colmap_path = mask_dir / f"{image_prefix}{idx + 1:05d}{m_path.suffix}"
            shutil.copy(m_path, copied_mask_colmap_path)
            copied_masks_paths.append(copied_mask_colmap_path)

        nn_flag = "" if not nearest_neighbor else ":flags=neighbor"
        downscale_chains = [f"[t{i}]scale=iw/{2 ** i}:ih/{2 ** i}{nn_flag}[out{i}]" for i in range(num_downscales + 1)]
        downscale_dirs = [Path(str(mask_dir) + (f"_{2 ** i}" if i > 0 else "")) for i in range(num_downscales + 1)]
        ...
        for framenum in range(1, (1 if same_dimensions else num_frames) + 1):
            framename = f"{image_prefix}%05d" if same_dimensions else f"{image_prefix}{framenum:05d}"
            ffmpeg_cmd = f'ffmpeg -y -noautorotate -i "{mask_dir / f"{framename}{copied_masks_paths[0].suffix}"}" -q:v 2 '
            crop_cmd = ""
            if crop_border_pixels is not None:
                crop_cmd = f"crop=iw-{crop_border_pixels * 2}:ih-{crop_border_pixels * 2}[cropped];[cropped]"
            elif crop_factor != (0.0, 0.0, 0.0, 0.0):
            ...
            downscale_cmd = f' -filter_complex "{select_cmd}{crop_cmd}{downscale_chain}"' + "".join(
                [
                    f' -map "[out{i}]" "{downscale_dirs[i] / f"{framename}{copied_masks_paths[0].suffix}"}"'
                    for i in range(num_downscales + 1)
                ]
            )
            ...
            if verbose:
                CONSOLE.log(f"... {ffmpeg_cmd}")
            run_command(ffmpeg_cmd, verbose=verbose)
        for u in range(len(copied_masks_paths)):
            image = Image.open(copied_masks_paths[u])
            new_image = str(copied_masks_paths[u])+".png"
            # new_image = str(copied_masks_paths[u]).split(".")[0]+".jpg"+".png"
            image.save(new_image)
            os.remove(str(copied_masks_paths[u]))

        if num_frames == 0:
            CONSOLE.log("[bold red]:skull: No usable images in the data folder.")
        else:
            CONSOLE.log(f"[bold green]:tada: Done copying MASKS with prefix '{image_prefix}'.")
...
def copy_images(
    ...
    with status(msg="[bold yellow]Copying images...", spinner="bouncingBall", verbose=verbose):
        ...
        if os.path.exists(image_dir.parent.parent / "mask"):
            dir_mask = image_dir.parent.parent / "mask"
            if len(os.listdir(str(dir_mask))) == len(os.listdir(str(data))):
                mask_path = list_images(dir_mask)
                print("we have mask for colmap!")
            else:
                raise RuntimeError("Number of masks is not equal to images")
        else:
            mask_path = None
            print("YOU DO NOT HAVE MASK FOLDER FOR COLMAP")
...
def save_mask(
    ...
    """
    #First copy original images to "images" folder for getting masks
    copy_before_mask(image_dir2, image_dir)
